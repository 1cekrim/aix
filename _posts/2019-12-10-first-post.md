...
| 김현수 | 컴퓨터소프트웨어학부 | 2019027192 | 1cekrim.dev@gmail.com | 코드 구현, 블로그 작성 |
...
영화 '마이너리티 리포트'에는 범죄가 일어나기 전에 범죄를 예측해 범죄자를 단죄하는 최첨단 치안 시스템이 등장합니다.  
이 시스템은 범죄가 일어날 시간과 장소, 범행을 저지를 사람들까지 예측할 수 있고, 특수경찰들이 미래의 범죄자들을 체포합니다.  
이렇게 범죄를 예측하는 도구를 '프리크라임'이라고 합니다. 우리는 여기서 아이디어를 얻어, 2019년 판 ‘프리크라임’을 만들기로 했습니다.
이번 게시글에서 2015년부터 2018년까지의 보스턴에서 발생한 범죄 데이터를 바탕으로. 언제 어디서 어떤 범죄가 일어날지 예측하는 시스템을 만들어 보고자 합니다.  
...
저희는 인공신경망을 이용해 4년간의 보스턴의 범죄 데이터를 분석해, 간단한 정보를 넣어줬을 때 어떤 범죄가 발생할지 예측하는  데 성공하는 것을 최종 목표로 선택했습니다.
...
학습 과정에서의 loss 변화, 정확도 변화 등을 보고 싶다면 tensorboard라는 프로그램을 이용해야 합니다.  
...
36개로 줄이는 데 성공했습니다!  
이제 비슷한 범죄끼리 묶어 범죄의 종류를 5~10개 정도로 줄이는 것이 좋겠습니다.  
...
6개로 줄이는 데 성공했습니다!  
그래프로 보면, 데이터의 불균형이 많이 해결된 것이 보입니다.  
...
예를 들어, 데이터 10만개 중 9만개가 a클래스이고, 1만개가 b클래스라면 입력에 상관없이 a클래스라고 데이터를 분류하는 이상한 모델이라도 90%의 정확도를 보이게 됩니다.  
그래서 보통 머신 러닝 알고리즘들은 데이터셋에서 각 클래스의 개수가 동일할 때 좋은 성능을 보여줍니다.  

데이터 불균형 문제를 해결하기 위해, Random undersampling이라는 기법을 사용할 것입니다.  
Random undersampling은 간단합니다. 그냥 가장 적은 개수의 클래스(저희의 경우 Other)의 개수 만큼 다른 클래스들의 데이터들을 무작위로 샘플링 하면 됩니다.
...
<style>.embed-container { position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; } .embed-container iframe, .embed-container object, .embed-container embed { position: absolute; top: 0; left: 0; width: 100%; height: 100%; }</style><div class='embed-container'><iframe src='https://www.youtube.com/embed//f9nWKTpj4qo' frameborder='0' allowfullscreen></iframe></div>